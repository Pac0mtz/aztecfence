...
function fenceStyleName(img: (typeof galleryImages)[number]) {
  const src = img.src.toLowerCase();
  const alt = img.alt;
  switch (img.category) {
    case "Aluminum":
      return "Ornamental Aluminum Fence";
    case "Vinyl Privacy":
      return "Vinyl Privacy Fence";
    case "Vinyl Picket":
      return "Vinyl Picket Fence";
    case "Wood Picket":
      return "Wood Picket Fence";
    case "Wood Board On Board":
      return alt.includes("Horizontal") ? "Horizontal Cedar Board-on-Board Fence" : "Wood Board-on-Board Fence";
    case "Wood Solid Privacy":
      return "Wood Solid Privacy Fence";
    case "Chain Link":
      if (alt.includes("Security")) return "Chain Link Security Gate";
      if (src.includes("commercial")) return "Commercial Chain Link Fence";
      return "Chain Link Fence";
    case "Commercial":
      if (alt.includes("Vinyl") && alt.includes("Gate") && !alt.includes("Fence")) return "Commercial Vinyl Privacy Gate";
      if (alt.includes("Vinyl")) return "Commercial Vinyl Privacy Fence & Gate";
      if (alt.includes("Gate")) return "Commercial Ornamental Fence & Gate";
      return "Commercial Ornamental Fence";
    case "Gates":
      if (src.includes("steel-gate-frames")) return "Steel Gate Frame on Wood";
      if (src.includes("gate-3139")) return "Custom Driveway Gate";
      if (src.includes("asset_4")) return "Ornamental Aluminum Fence & Gate";
      return "Custom Steel Gate";
    default:
      return alt;
  }
}
            ...
            {filtered.map((img, idx) => (
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/35 to-transparent px-2.5 pt-8 pb-2">
                  <p className="text-white text-xs font-semibold leading-snug">{fenceStyleName(img)}</p>
                </div>
                {/* Hover zoom icon */}
          ...
          >
            ...
            <div className="absolute top-0 left-0 right-0 flex items-center justify-between px-4 py-3 z-50">
              <p className="text-white/60 text-sm">
                {filtered[lightboxIdx] ? fenceStyleName(filtered[lightboxIdx]) : ""} — {lightboxIdx + 1} / {filtered.length}
              </p>
              <button
            ...
            <motion.img
              ...
              transition={{ duration: 0.2 }}
              src={lightbox}
              alt={filtered[lightboxIdx] ? fenceStyleName(filtered[lightboxIdx]) : "Gallery"}
            ...
            <p className="absolute bottom-4 text-white/70 text-sm font-medium">
              {filtered[lightboxIdx] ? fenceStyleName(filtered[lightboxIdx]) : ""}
            </p>
          </motion.div>
        )}
      </AnimatePresence>
